#ifndef MY_OTA_Firmware_Update_h
#define MY_OTA_Firmware_Update_h
// Local includes.
...
class MY_OTA_Firmware_Update : public IAPI_Implementation {
  public:
    /// @brief Constructor
    MY_OTA_Firmware_Update()
      ...
      , m_ota(std::bind(&MY_OTA_Firmware_Update::Publish_Chunk_Request, this, std::placeholders::_1, std::placeholders::_2), std::bind(&MY_OTA_Firmware_Update::Firmware_Send_State, this, std::placeholders::_1, std::placeholders::_2), std::bind(&MY_OTA_Firmware_Update::Firmware_OTA_Unsubscribe, this))
#else
      , m_ota(MY_OTA_Firmware_Update::staticPublishChunk, MY_OTA_Firmware_Update::staticFirmwareSend, MY_OTA_Firmware_Update::staticUnsubscribe)
...
#if THINGSBOARD_ENABLE_STL
        const Attribute_Request_Callback fw_request_callback(std::bind(&MY_OTA_Firmware_Update::Firmware_Shared_Attribute_Received, this, std::placeholders::_1), callback.Get_Timeout(), std::bind(&MY_OTA_Firmware_Update::Request_Timeout, this), array + 0U, array + OTA_ATTRIBUTE_KEYS_AMOUNT);
#else
        const Attribute_Request_Callback fw_request_callback(MY_OTA_Firmware_Update::onStaticFirmwareReceived, callback.Get_Timeout(), MY_OTA_Firmware_Update::onStaticRequestTimeout, array + 0U, array + OTA_ATTRIBUTE_KEYS_AMOUNT);
#endif // THINGSBOARD_ENABLE_STL
#else
#if THINGSBOARD_ENABLE_STL
        /* Thêm các shared attribute do người dùng định nghĩa*/
        // for(int i = 0 ;i < 5 ; i++){
        //     this->user_attribute.push_back(array[i]);
        // }

        const Attribute_Request_Callback<OTA_ATTRIBUTE_KEYS_AMOUNT + MAX_USER_SHARED_ATTRIBUTE> fw_request_callback(std::bind(&MY_OTA_Firmware_Update::Firmware_Shared_Attribute_Received,
        this, std::placeholders::_1), callback.Get_Timeout(), 
        std::bind(&MY_OTA_Firmware_Update::Request_Timeout, this), 
        this->user_attribute.begin(), this->user_attribute.end());
#else
        const Attribute_Request_Callback<OTA_ATTRIBUTE_KEYS_AMOUNT> fw_request_callback(MY_OTA_Firmware_Update::onStaticFirmwareReceived, callback.Get_Timeout(), MY_OTA_Firmware_Update::onStaticRequestTimeout, array + 0U, array + OTA_ATTRIBUTE_KEYS_AMOUNT);
#endif // THINGSBOARD_ENABLE_STL
    ...
    bool Subscribe_Firmware_Update(OTA_Update_Callback const & callback) {
...
#if THINGSBOARD_ENABLE_DYNAMIC
#if THINGSBOARD_ENABLE_STL
        const Shared_Attribute_Callback fw_update_callback(std::bind(&MY_OTA_Firmware_Update::Firmware_Shared_Attribute_Received, this, std::placeholders::_1), array + 0U, array + OTA_ATTRIBUTE_KEYS_AMOUNT);
#else
        const Shared_Attribute_Callback fw_update_callback(MY_OTA_Firmware_Update::onStaticFirmwareReceived, array + 0U, array + OTA_ATTRIBUTE_KEYS_AMOUNT);
#endif // THINGSBOARD_ENABLE_STL
#else
#if THINGSBOARD_ENABLE_STL
        /* Thêm các shared attribute do người dùng định nghĩa*/
        ...
        for(auto it:this->user_attribute){
            Serial.println(" ------------------------------------------ ");
            Serial.println(it);
        }
        const Shared_Attribute_Callback<OTA_ATTRIBUTE_KEYS_AMOUNT + MAX_USER_SHARED_ATTRIBUTE> fw_update_callback(std::bind(&MY_OTA_Firmware_Update::Firmware_Shared_Attribute_Received, 
        this, std::placeholders::_1), this->user_attribute.begin(), this->user_attribute.end());
#else
        const Shared_Attribute_Callback<OTA_ATTRIBUTE_KEYS_AMOUNT> fw_update_callback(MY_OTA_Firmware_Update::onStaticFirmwareReceived, array + 0U, array + OTA_ATTRIBUTE_KEYS_AMOUNT);
#endif // THINGSBOARD_ENABLE_STL
#endif //THINGSBOARD_ENABLE_DYNAMIC
    ...
    /// @brief Thêm key attribute của người dùng
    void Add_Shared_Attribute(const char* key){
        this->user_attribute.push_back(key);
    }

    void Set_User_Shared_Attribute_Callback(std::function<bool(const JsonObjectConst &)> m_userCallback){
        this->m_userCallback = m_userCallback;
    }
  private:
    ...
    static MY_OTA_Firmware_Update                                               *m_subscribedInstance;
#endif // !THINGSBOARD_ENABLE_STL
...
#if !THINGSBOARD_ENABLE_DYNAMIC
    Shared_Attribute_Update<1U, OTA_ATTRIBUTE_KEYS_AMOUNT + MAX_USER_SHARED_ATTRIBUTE, Logger>           m_fw_attribute_update = {};               // API implementation to be informed if needed fw attributes have been updated
    Attribute_Request<1U, OTA_ATTRIBUTE_KEYS_AMOUNT + MAX_USER_SHARED_ATTRIBUTE, Logger>                 m_fw_attribute_request = {};              // API implementation to request the needed fw attributes to start updating
#else
...
#endif // !THINGSBOARD_ENABLE_DYNAMIC
/* Các Shared do người dùng định nghĩa*/

std::vector<const char *> user_attribute;
// Callback user
std::function<bool(const JsonObjectConst &)> m_userCallback;
};

#if !THINGSBOARD_ENABLE_STL
MY_OTA_Firmware_Update *MY_OTA_Firmware_Update::m_subscribedInstance = nullptr;
#endif

#endif // MY_OTA_Firmware_Update_h
